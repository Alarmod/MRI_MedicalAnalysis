...
//Linebase construct
//-----------------------------------------------------------------------------
Linebase::Linebase(Pointbase* sp, Pointbase* ep, Type type, long int & l_id) :_type(type)
{
  _endp[0] = sp;
  _endp[1] = ep;
...
BDMFile::BDMFile(string filename, long int & l_id, long int & p_id, bool parse)
{
  _xmin = _ymin = 1.0e38;
  _xmax = _ymax = -1.0e38;
  ...
  if (parse) read_bdm(filename, l_id, p_id);
  else read_bdm(filename, l_id, p_id, false);
  _prefix = filename;
...
//----------------------------------------------------------------------------
void BDMFile::read_bdm(const string& filename, long int & l_id, long int & p_id)
{
  ...
  for (unsigned int j = 0; j < _ncontours; j++)
  {
    for (i = 1; i <= _nVertices[j]; i++)
    {
      ...
      type = SINPUT;
      Linebase* line = new Linebase(_points[sid], _points[eid], type, l_id);
      _edges[l_id] = line;
    }
    num += _nVertices[j];
  }
...
//----------------------------------------------------------------------------
void BDMFile::read_bdm(const string& filename, long int & l_id, long int & p_id, bool parse)
{
...
//----------------------------------------------------------------------------
SPolygon::SPolygon(string filename, long int & l_id, long int & p_id, bool parse) : BDMFile(filename, l_id, p_id, parse)
{
  initializate();
  _debug = false;
}

SPolygon::SPolygon(BDMFile & BDM) : BDMFile(BDM)
{
  initializate();
  _debug = false;
}
...
//----------------------------------------------------------------------------
unsigned int SPolygon::prev(unsigned int i)
{
...
//----------------------------------------------------------------------------
unsigned int SPolygon::next(unsigned int i)
{
  unsigned int j(0), prevLoop(0), currentLoop(0);
...
//----------------------------------------------------------------------------
void SPolygon::rotate(double theta)
{
  PointbaseMap::iterator it = _points.begin();
  for (; it != _points.end(); it++)
...
//----------------------------------------------------------------------------  
void SPolygon::addDiagonal(unsigned int i, unsigned int j, long int & l_id)
{
  Type type = INSERT;
  Linebase* diag = new Linebase(_points[i], _points[j], type, l_id);
  _edges[diag->id()] = diag;
...
//Handle start vertex
//----------------------------------------------------------------------------
void SPolygon::handleStartVertex(unsigned int i, long int & l_id)
{
...
//Handle end vertex
//----------------------------------------------------------------------------
void SPolygon::handleEndVertex(unsigned int i, long int & l_id)
{
  double y = _points[i]->y;
  _edgebst.InOrder(UpdateKey, y);
  ...
  if (_points[helper]->type == MERGE) addDiagonal(i, helper, l_id);
...
//Handle split vertex
//----------------------------------------------------------------------------
void SPolygon::handleSplitVertex(unsigned int i, long int & l_id)
{
...
//Handle merge vertex
//----------------------------------------------------------------------------
void SPolygon::handleMergeVertex(unsigned int i, long int & l_id)
{
  double x = _points[i]->x, y = _points[i]->y;
  _edgebst.InOrder(UpdateKey, y);
...
//----------------------------------------------------------------------------
void SPolygon::handleRegularVertexDown(unsigned int i, long int & l_id)
{
  double y = _points[i]->y;
  _edgebst.InOrder(UpdateKey, y);
...
//----------------------------------------------------------------------------
void SPolygon::handleRegularVertexUp(unsigned int i, long int & l_id)
{
  double x = _points[i]->x, y = _points[i]->y;
  _edgebst.InOrder(UpdateKey, y);
...
//----------------------------------------------------------------------------
void SPolygon::partition2Monotone(long int & l_id)
{
  ...
  while (!_qpoints.empty())
  {
    ...
    switch (vertex.type)
    {
    case START:        handleStartVertex(id, l_id);       break;
    case END:          handleEndVertex(id, l_id);         break;
    case MERGE:        handleMergeVertex(id, l_id);       break;
    case SPLIT:        handleSplitVertex(id, l_id);       break;
    case REGULAR_UP:   handleRegularVertexUp(id, l_id);   break;
    case REGULAR_DOWN: handleRegularVertexDown(id, l_id); break;
    default:
      cout << "No duplicated points please! poly2tri stopped\n";
      exit(1); break;
    }
...
//----------------------------------------------------------------------------
double SPolygon::angleCosb(double *pa, double *pb, double *pc)
{
...
//----------------------------------------------------------------------------
unsigned int SPolygon::selectNextEdge(Linebase* edge)
{
...
//----------------------------------------------------------------------------
void  SPolygon::triangulateMonotone(Monopoly& mpoly)
{
...
////----------------------------------------------------------------------------
void SPolygon::triangulation(long int & l_id)
{
#if defined(WIN32)
...
void SPolygon::setDebugOption(bool debug)
{
  _debug = debug;
  if (_debug && !_logfile.is_open())
  {
