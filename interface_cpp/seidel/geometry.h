...
public:
  //constructors and destructor	  
  Linebase();
  Linebase(Pointbase* ep1, Pointbase* ep2, Type type, long int & l_id);
...
{
public:
  BDMFile(std::string filename, long int & l_id, long int & p_id, bool parse = true);
  BDMFile();
  ~BDMFile();
  ...
  bool   readline(ifstream& filename, std::string& result);
  int    countfield(const std::string& source, char delim);
  std::string findfield(std::string& source);
  bool   is_exist(double, double);
  void   read_bdm(const std::string& filename, long int & l_id, long int & p_id);
  void   read_bdm(const std::string& filename, long int & l_id, long int & p_id, bool parse);
  ...
  std::string             _prefix;                    //prefix of associated polygon bdm file; 
};

class SPolygon : public BDMFile
{
public:
  //constructor and destructor   
  SPolygon(std::string filename, long int & l_id, long int & p_id, bool parse = true);
  SPolygon(BDMFile & BDM);
  ~SPolygon();

  // main member function for polygon triangulation; 	
  void         partition2Monotone(long int &);
  void         searchMonotones();
  void         triangulation(long int &);
...
private:
  ...
  void         handleStartVertex(unsigned int, long int &);
  void         handleEndVertex(unsigned int, long int &);
  void         handleSplitVertex(unsigned int, long int &);
  void         handleMergeVertex(unsigned int, long int &);
  void         handleRegularVertexUp(unsigned int, long int &);
  void         handleRegularVertexDown(unsigned int, long int &);

  //add diagonal between two vertices;
  void         addDiagonal(unsigned int i, unsigned int j, long int & l_id);
