...
global_visualize_conf_thresh=0.01
...
def plot_images_and_export_data(images, batch_idx, cls, bboxes=np.zeros(0, dtype=np.float32), masks=np.zeros(0, dtype=np.uint8), kpts=np.zeros((0, 51), dtype=np.float32), paths=None, fname='images.jpg', names=None, on_plot=None, conf_thresh=0.25, show_labels=True, export_data=True): 
...
def save_results(results, mask, imgsz_val, brain=None, brain_imgsz_val=None, too_many_fragments_warning=300, brain_color=(255, 127, 255), target_color=(127, 255, 255), thickness_mod=1, use_computed_brain_mask_for_prediction_zone_correction=True, erode_level=0, erode_mask_size=5): 
    ...
    if brain != None: 
       if len(results) > 0: 
          ...
          results_brain = brain.predict(workers=global_workers, verbose=False, name="temp", batch=4, source=brains_path, imgsz=brain_imgsz_val, save=False, conf=0.25, show_labels=False, show_conf=False, boxes=False, max_det=1)
...
# Based on original Yolo codes
def internal_validate(exp_name, big_data, imgsz_val, model_path, dataset_info, batch_size=1, visualize_conf_thresh=global_visualize_conf_thresh, visualize_show_labels = False, iou_val=0.6, conf_val=0.01, max_det_val=300): 
    ...
    with torch.no_grad(): 
      ...
      for batch_i, batch_with_data in enumerate(bar): 
          ...
          for si, (pred, proto) in enumerate(zip(preds[0], preds[1])):
              idx = batch_with_data['batch_idx'] == si
              cls = batch_with_data['cls'][idx]
              bbox = batch_with_data['bboxes'][idx]
              #print(f"Process '{batch_with_data['im_file'][si]}'")
              nl, npr = cls.shape[0], pred.shape[0]
              shape = batch_with_data['ori_shape'][si]
              correct_masks = torch.zeros(npr, validator.niou, dtype=torch.bool, device=validator.device)  # init
              correct_bboxes = torch.zeros(npr, validator.niou, dtype=torch.bool, device=validator.device)  # init
              validator.seen += 1

              if npr == 0: 
                  if nl:
                     validator.stats.append((correct_bboxes, correct_masks, *torch.zeros((2, 0), device=validator.device), cls.squeeze(-1)))
                     if validator.args.plots:
                        validator.confusion_matrix.process_batch(detections=None, labels=cls.squeeze(-1))
                  continue

              # Masks
              midx = [si] if validator.args.overlap_mask else idx
              gt_masks = batch_with_data['masks'][midx]
              pred_masks = validator.process(proto, pred[:, 6:], pred[:, :4], shape=batch_with_data['img'][si].shape[1:])

              # Predictions
              if validator.args.single_cls: 
                 pred[:, 5] = 0
              predn = pred.clone()
              ops.scale_boxes(batch_with_data['img'][si].shape[1:], predn[:, :4], shape, ratio_pad=batch_with_data['ratio_pad'][si])  # native-space pred

              ratio_pad_value = batch_with_data['ratio_pad'][si]
              #print(f"ratio_pad_value: {ratio_pad_value}")

              # Evaluate
              if nl:
                 height, width = batch_with_data['img'].shape[2:]
                 tbox = ops.xywh2xyxy(bbox) * torch.tensor((width, height, width, height), device=validator.device)  # target boxes
                 ops.scale_boxes(batch_with_data['img'][si].shape[1:], tbox, shape, ratio_pad=ratio_pad_value)    # native-space labels
                 labelsn = torch.cat((cls, tbox), 1)  # native-space labels

                 correct_bboxes = validator._process_batch(predn, labelsn)
                 correct_masks = validator._process_batch(predn, labelsn, pred_masks, gt_masks, overlap=validator.args.overlap_mask, masks=True)

                 if validator.args.plots:
                    validator.confusion_matrix.process_batch(predn, labelsn)

              # Append correct_masks, correct_boxes, pconf, pcls, tcls
              validator.stats.append((correct_bboxes, correct_masks, pred[:, 4], pred[:, 5], cls.squeeze(-1)))

              # Save data to plotting
              validator.plot_masks.append(pred_masks[:validator.args.max_det])

          # plot_val_samples
          plot_images_and_export_data(batch_with_data['img'], batch_with_data['batch_idx'], batch_with_data['cls'].squeeze(-1), batch_with_data['bboxes'], batch_with_data['masks'], paths=batch_with_data['im_file'], fname=validator.save_dir / f'val_batch{batch_i}_labels.jpg', names=validator.names, on_plot=validator.on_plot, conf_thresh=visualize_conf_thresh, show_labels=visualize_show_labels, export_data=False)

          # plot_predictions
          plot_images_and_export_data(batch_with_data['img'], *output_to_target(preds[0], max_det=validator.args.max_det), torch.cat(validator.plot_masks, dim=0) if len(validator.plot_masks) else validator.plot_masks, paths=batch_with_data['im_file'], fname=validator.save_dir / f'val_batch{batch_i}_pred.jpg', names=validator.names, on_plot=validator.on_plot, conf_thresh=visualize_conf_thresh, show_labels=visualize_show_labels, export_data=True)
...
def runtime_func(big_data, default_validate, generate_data, generate_data_hq):
    if default_validate: 
       ...
       if os.path.exists(source_val) and os.listdir(source_val): 
          results = model_brain.predict(workers=global_workers, overlap_mask=global_overlap_mask, single_cls=global_single_cls, save_json=global_save_json, mask_ratio=global_mask_ratio, retina_masks=global_retina_masks, half=global_half, rect=True, verbose=False, name="predict_t2_brain_" + str(brain_and_ischemia_imgsz) + "_default" + ("_big" if big_data else ""), batch=44, source=source_val, imgsz=brain_and_ischemia_imgsz, save=True, conf=(global_hq_threshold if generate_data_hq else def_tresh), iou=global_iou, show_labels=big_data, boxes=big_data, show_conf=big_data, max_det=1)
       ...
       if os.path.exists(source_val) and os.listdir(source_val): 
          results = model.predict(workers=global_workers, overlap_mask=global_overlap_mask, single_cls=global_single_cls, save_json=global_save_json, mask_ratio=global_mask_ratio, retina_masks=global_retina_masks, half=global_half, rect=True, verbose=False, name="predict_t2_ischemia_" + str(brain_and_ischemia_imgsz) + "_default" + ("_big" if big_data else ""), batch=4, source=source_val, imgsz=brain_and_ischemia_imgsz, save=True, conf=(global_hq_threshold if generate_data_hq else def_tresh), iou=global_iou, show_labels=big_data, boxes=big_data, show_conf=big_data)
       ...
       if os.path.exists(source_val) and os.listdir(source_val): 
          results = model_brain.predict(workers=global_workers, overlap_mask=global_overlap_mask, single_cls=global_single_cls, save_json=global_save_json, mask_ratio=global_mask_ratio, retina_masks=global_retina_masks, half=global_half, rect=True, verbose=False, name="predict_adc_brain_" + str(brain_and_ischemia_imgsz) + "_default" + ("_big" if big_data else ""), batch=44, source=source_val, imgsz=brain_and_ischemia_imgsz, save=True, conf=(global_hq_threshold if generate_data_hq else def_tresh), iou=global_iou, show_labels=big_data, boxes=big_data, show_conf=big_data, max_det=1)
       ...
       if os.path.exists(source_val) and os.listdir(source_val): 
          results = model.predict(workers=global_workers, overlap_mask=global_overlap_mask, single_cls=global_single_cls, save_json=global_save_json, mask_ratio=global_mask_ratio, retina_masks=global_retina_masks, half=global_half, rect=True, verbose=False, name="predict_adc_ischemia_" + str(brain_and_ischemia_imgsz) + "_default" + ("_big" if big_data else ""), batch=4, source=source_val, imgsz=brain_and_ischemia_imgsz, save=True, conf=(global_hq_threshold if generate_data_hq else def_tresh), iou=global_iou, show_labels=big_data, boxes=big_data, show_conf=big_data)
       ...
       if os.path.exists(source_val) and os.listdir(source_val): 
          results = model_brain.predict(workers=global_workers, overlap_mask=global_overlap_mask, single_cls=global_single_cls, save_json=global_save_json, mask_ratio=global_mask_ratio, retina_masks=global_retina_masks, half=global_half, rect=True, verbose=False, name="predict_swi_brain_" + str(brain_and_ischemia_imgsz) + "_default" + ("_big" if big_data else ""), batch=44, source=source_val, imgsz=brain_and_ischemia_imgsz, save=True, conf=(global_hq_threshold if generate_data_hq else def_tresh), iou=global_iou, show_labels=big_data, boxes=big_data, show_conf=big_data, max_det=1)
       ...
       if os.path.exists(source_val) and os.listdir(source_val): 
          results = model.predict(workers=global_workers, overlap_mask=global_overlap_mask, single_cls=global_single_cls, save_json=global_save_json, mask_ratio=global_mask_ratio, retina_masks=global_retina_masks, half=global_half, rect=True, verbose=False, name="predict_swi_msc_" + str(msk_imgsz) + "_default" + ("_big" if big_data else ""), batch=6, source=source_val, imgsz=msk_imgsz, save=True, conf=(global_hq_threshold if generate_data_hq else def_tresh), iou=global_iou, show_labels=False, boxes=False, show_conf=False)
          save_results(results, "_msk", brain=model_brain, brain_imgsz_val=brain_and_ischemia_imgsz, imgsz_val=msk_imgsz, thickness_mod=2)
    else: 
       internal_validate(exp_name="val_t2_brain_" + str(brain_and_ischemia_imgsz) + "_new", big_data=big_data, imgsz_val=brain_and_ischemia_imgsz, model_path="./runs/segment/t2_brain_" + str(brain_and_ischemia_imgsz) + "/weights/best.pt", dataset_info="data_t2_brain.yaml", batch_size=4, visualize_conf_thresh=global_visualize_conf_thresh, visualize_show_labels=True, iou_val=global_iou, conf_val=0.01, max_det_val=1)
       internal_validate(exp_name="val_t2_ischemia_" + str(brain_and_ischemia_imgsz) + "_new", big_data=big_data, imgsz_val=brain_and_ischemia_imgsz, model_path="./runs/segment/t2_ischemia_" + str(brain_and_ischemia_imgsz) + "_augmented/weights/best.pt", dataset_info="data_t2_ischemia.yaml", batch_size=4, visualize_conf_thresh=global_visualize_conf_thresh, visualize_show_labels=True, iou_val=global_iou, conf_val=0.01)

       internal_validate(exp_name="val_adc_brain_" + str(brain_and_ischemia_imgsz) + "_new", big_data=big_data, imgsz_val=brain_and_ischemia_imgsz, model_path="./runs/segment/adc_brain_" + str(brain_and_ischemia_imgsz) + "/weights/best.pt", dataset_info="data_adc_brain.yaml", batch_size=4, visualize_conf_thresh=global_visualize_conf_thresh, visualize_show_labels=True, iou_val=global_iou, conf_val=0.01, max_det_val=1)
       internal_validate(exp_name="val_adc_ischemia_" + str(brain_and_ischemia_imgsz) + "_new", big_data=big_data, imgsz_val=brain_and_ischemia_imgsz, model_path="./runs/segment/adc_ischemia_" + str(brain_and_ischemia_imgsz) + "_augmented/weights/best.pt", dataset_info="data_adc_ischemia.yaml", batch_size=4, visualize_conf_thresh=global_visualize_conf_thresh, visualize_show_labels=True, iou_val=global_iou, conf_val=0.01)

       internal_validate(exp_name="val_swi_brain_" + str(brain_and_ischemia_imgsz) + "_new", big_data=big_data, imgsz_val=brain_and_ischemia_imgsz, model_path="./runs/segment/swi_brain_" + str(brain_and_ischemia_imgsz) + "/weights/best.pt", dataset_info="data_swi_brain.yaml", batch_size=4, visualize_conf_thresh=global_visualize_conf_thresh, visualize_show_labels=True, iou_val=global_iou, conf_val=0.01, max_det_val=1)
       internal_validate(exp_name="val_swi_msc_" + str(msk_imgsz) + "_new", big_data=big_data, imgsz_val=msk_imgsz, model_path="./runs/segment/swi_msc_mod_" + str(msk_imgsz) + "_augmented/weights/best.pt", dataset_info="data_swi_msc.yaml", batch_size=4, visualize_conf_thresh=global_visualize_conf_thresh, visualize_show_labels=False, iou_val=global_iou, conf_val=0.01)
